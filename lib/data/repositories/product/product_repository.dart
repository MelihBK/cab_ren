...
import 'package:cab_ren/firebase_storage_service.dart';
import 'package:cab_ren/utils/exceptions/firebase_auth_exceptions.dart';
...
/// Repository for maintaining product-related data and operations
  ...
  Future<List<ProductModel>> getProductsForCategory({required String categoryId, int limit = -1}) async {
    try {
      ...
      QuerySnapshot productCategoryQuery = limit == -1
          ? await _db.collection('Products').where('Brand.Id',isEqualTo: categoryId).get()
          : await _db.collection('Products').where('Brand.Id', isEqualTo: categoryId).limit(limit).get();
